    ...
    const handleUpload = async () => {
        ...
        try {
            const response = await fetch("/api/avatar", {
                ...
                body: JSON.stringify({ userId: user.id, file, action: 'upload' }),
            });
    ...
    return (
        <div className="min-w-[450px] p-2 w-full flex flex-col justify-center text-xs">
            <div className="w-full flex justify-center items-end pt-10 relative">
                ...
                <Button variant="ghost" className="w-fit h-fit p-2 text-xs cursor-pointer">
                ...
                <Button variant="ghost" className="w-fit h-fit p-2 text-xs cursor-pointer" onClick={handleRemove}>
